...
if (isCheckingAuth) {
    ...
    return (
        ...
        <button onClick={() => setIsMuted(!isMuted)} className="fixed bottom-4 left-4 md:bottom-8 md:left-8 z-[9999] bg-white/90 backdrop-blur-md w-12 h-12 md:w-16 md:h-16 rounded-full flex items-center justify-center shadow-[0_4px_0_#d1d5db] border-2 border-gray-200 text-gray-700 hover:text-blue-500 hover:scale-110 active:translate-y-[4px] active:shadow-none transition-all">
            ...
            {view === 'profile' && <ProfileSettings setView={setView} user={user} userData={userData} handleSignOut={handleSignOut} />}
...
function ProfileSettings({ setView, user, userData, handleSignOut }) {
            ...
            <div className="bg-white p-6 md:p-10 rounded-[2.5rem] shadow-[0_15px_40px_rgba(0,0,0,0.1)] border-4 border-white max-w-md w-full max-h-[95vh] overflow-y-auto">
                ...
                <form onSubmit={handleSave} className="flex flex-col gap-4">
                   ...
                   <button type="submit" className="bg-gradient-to-b from-green-400 to-green-600 text-white font-black py-3 rounded-xl shadow-[0_6px_0_#166534] active:translate-y-[6px] active:shadow-none text-lg transition-all mt-4 uppercase tracking-wider">
                    ...
                    {/* ปุ่มออกจากระบบเพิ่มเข้ามาใหม่ */}
                    <button type="button" onClick={handleSignOut} className="bg-gradient-to-b from-red-400 to-red-500 text-white font-black py-3 rounded-xl shadow-[0_6px_0_#991b1b] active:translate-y-[6px] active:shadow-none text-lg transition-all mt-1 uppercase tracking-wider">
    ...
    return (
        <div className="flex flex-col gap-2 w-full">
            ...
            <div className="flex justify-center gap-1.5 md:gap-2 flex-wrap bg-white p-2 rounded-2xl border-2 border-gray-200 shadow-sm">
                <button onPointerDown={insertFraction} className="bg-blue-50 border-2 border-blue-200 text-blue-700 hover:bg-blue-100 rounded-xl px-2 py-1.5 md:px-3 md:py-2 font-bold text-xs md:text-sm active:translate-y-1 transition-all flex items-center whitespace-nowrap"><i className="fas fa-columns rotate-90 mr-1.5"></i>เศษส่วน</button>
                <button onPointerDown={e=>insertText(e,'•')} className="bg-purple-50 border-2 border-purple-200 text-purple-700 hover:bg-purple-100 rounded-xl px-2 py-1.5 md:px-3 md:py-2 font-bold text-xs md:text-sm active:translate-y-1 transition-all flex items-center whitespace-nowrap"><i className="fas fa-circle text-[8px] mr-1.5"></i>คูณ(•)</button>
                <button onPointerDown={e=>insertText(e,'x')} className="bg-green-50 border-2 border-green-200 text-green-700 hover:bg-green-100 rounded-xl px-2 py-1.5 md:px-3 md:py-2 font-black text-xs md:text-sm active:translate-y-1 transition-all whitespace-nowrap">ตัวแปร x</button>
                <button onPointerDown={e=>insertText(e,'+')} className="bg-gray-100 border-2 border-gray-300 text-gray-700 hover:bg-gray-200 rounded-xl px-3 py-1.5 md:px-4 md:py-2 font-black text-xs md:text-sm active:translate-y-1 transition-all">+</button>
                <button onPointerDown={e=>insertText(e,'-')} className="bg-gray-100 border-2 border-gray-300 text-gray-700 hover:bg-gray-200 rounded-xl px-3 py-1.5 md:px-4 md:py-2 font-black text-xs md:text-sm active:translate-y-1 transition-all">-</button>
                <button onPointerDown={clearEditor} className="bg-red-50 border-2 border-red-200 text-red-600 hover:bg-red-100 rounded-xl px-2 py-1.5 md:px-3 md:py-2 font-bold text-xs md:text-sm active:translate-y-1 transition-all flex items-center whitespace-nowrap"><i className="fas fa-trash-alt mr-1.5"></i>ล้าง</button>
            </div>
...
eng.handleFractionDivision = (targetCard) => {
        ...
        eng.setupDrag = (el, term, side, list, idx, role, parentFracTerm = null, mainList = null, mainIdx = null, sourceContext = null) => {
            const handleStart = (clientX, clientY, eOriginal) => {
                ...
                // ตรวจสอบจาก DOM แทนพิกัดหน้าจอ เพื่อแก้ปัญหาสมการยาวทะลุเส้นกึ่งกลาง
                if (elemBelow) {
                    let dropZone = elemBelow.closest('#engine-lhs, #engine-rhs');
                    if (dropZone) {
                        targetSide = dropZone.id === 'engine-lhs' ? 'lhs' : 'rhs';
                    } else {
                        let rect = pg.getBoundingClientRect(), midX = rect.left + rect.width/2;
                        targetSide = (endX > midX) ? 'rhs' : 'lhs';
                    }
                }
                ...
                if (isCrossMove) {
                    ...
                    if (targetContext !== null) {
                        let srcContext = eng.dragSrc.sourceContext;
...
eng.tryCombine = (targetWrapper) => {
            ...
            if (srcTerm.type === 'op' && (srcTerm.value === '+' || srcTerm.value === '-') && (targetTerm.type === 'group' || targetTerm.type === 'fraction')) {
                if (max - min === 1 && eng.dragSrc.idx < targetIdx) {
                ...
                if (!isMult) {
                    ...
                    if (max - min === 2 && (list[min+1].value === '+' || list[min+1].value === '-')) {
                        if (p1 && p2 && p1.v === p2.v) {
                            ...
                            let leftIdx = min; let rightIdx = max; let midOpIdx = min + 1;
                            ...
                            if (opLeftNode) { opLeftNode.value = opRightVal; }
                            else if (opRightVal === '-') { list.unshift(new eng.TermClass('op', '-')); }
                            eng.incrementMove(); eng.commitState(); eng.playTone('pop'); return;
                        }
                    ...
                    <div className="flex flex-col bg-white/60 backdrop-blur-md rounded-[2rem] p-2 md:p-4 border-4 border-white shadow-inner shrink-0 min-h-[60vh]">
                        ...
                        <div className="shrink-0 flex justify-center mt-3 md:mt-4">
                            <div className="flex items-center gap-2 md:gap-4 bg-white/95 px-3 py-2 md:px-5 md:py-3 rounded-full shadow-lg border-2 border-gray-200 whitespace-nowrap">
                                {/* ปุ่มเลื่อนซ้าย */}
                                <button onClick={() => document.getElementById('engine-playground').scrollBy({left: -200, behavior: 'smooth'})} className="text-blue-600 hover:text-blue-700 text-lg md:text-2xl active:scale-90 transition-transform bg-blue-50 hover:bg-blue-100 w-10 h-10 md:w-12 md:h-12 rounded-full flex items-center justify-center shadow-sm border border-blue-200">
                                    <i className="fas fa-chevron-left"></i>
                                </button>

                                <button onClick={() => engineRef.current.undo()} className="text-gray-500 hover:text-blue-600 text-lg md:text-2xl active:scale-90 transition-transform bg-gray-100 w-10 h-10 md:w-12 md:h-12 rounded-full flex items-center justify-center shadow-inner border border-gray-200">
                                    <i className="fas fa-undo"></i>
                                </button>
                                
                                <div className="text-gray-600 font-bold border-l-2 border-r-2 px-3 md:px-4 text-xs md:text-sm border-gray-200 flex items-center">
